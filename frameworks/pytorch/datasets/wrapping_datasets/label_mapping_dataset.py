...
class LabelListMappingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        try:
            if isinstance(self.base_dataset[idx], str) : self.base_dataset[idx] = [self.base_dataset[idx]]
            else : base_output = list(self.base_dataset[idx])
        except:
            base_output = [self.base_dataset[idx]]

        for _idx in self.indexs:
            if isinstance(base_output[_idx], str) : base_output[_idx] = [base_output[_idx]]

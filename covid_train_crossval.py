...
def main(model_type):
    ...
    base_train_dataset = build_covid_dataset(data_params.train_scp_path, param_loader_params, mel_feats=mel_feats, basepath=data_params.wavs_path, labels_filepath=data_params.train_labels_path)
    base_valid_dataset = build_covid_dataset(data_params.valid_scp_path, param_loader_params, mel_feats=mel_feats, basepath=data_params.wavs_path, labels_filepath=data_params.valid_labels_path)

    full_dataset = data.ConcatDataset([base_train_dataset, base_valid_dataset])
    targets = [full_dataset[i][2] for i in range(len(full_dataset))]
    
    pos_idx = [t for t in targets if t == 1]
    neg_idx = [t for t in targets if t == 0]
    random.shuffle(pos_idx)
    random.shuffle(neg_idx)
    
    splits_pos_idx = np.array_split(pos_idx, 5)
    splits_neg_idx = np.array_split(neg_idx, 5)

    splits_list = [data.Subset(full_dataset, pos.tolist() + neg.tolist()) for pos, neg in zip(splits_pos_idx, splits_neg_idx)]

    collate_fn = build_covid_collate()

    folds = list(set(combinations(range(5), 1)))
    num_folds = len(folds)
    ...
    for fold, indxs in enumerate(folds):
        ...
        train_lists = [splits_list[indx] for indx in range(len(splits_list)) if indx not in indxs]
        val_lists = [splits_list[indx] for indx in indxs]

        train_dataset = data.ConcatDataset(train_lists)
        valid_dataset = val_lists[0]
